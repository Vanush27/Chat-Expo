...
const ChatScreen = ({ route }: any) => {
	...
	const [emojiVal, setEmojiVal] = useState<any>();
	...
	// const handleOpenEmoji = () => navigation.navigate('EmojiScreen');

	useLayoutEffect(() => {
		const msgQuery = query(collection(firestoreDB, 'chats', room?._id, 'messages'), orderBy('timeStamp', 'asc'));

		const unsubscribe = onSnapshot(msgQuery, (querySnap) => {
			const upMsg: any = querySnap.docs.map((doc) => doc.data());
			setMessages(upMsg);
			setIsLoading(false);
		});
	...
	const handleSelect = (emojiObject: EmojiType) => {
		...
		setEmojiVal(emojiTest.emoji);
		console.log(emojiTest, 'emojiTest');
						...
						<View className="w-full flex-row items-center justify-center px-8">
							<View className="bg-gray-200 rounded-2xl px-4 space-x-4 py-2 flex-row items-center justify-center">
								<TouchableOpacity onPress={handleKeyboardOpen}>
									<Entypo onPress={handleOpenEmoji} name="emoji-happy" size={24} color="#555" />

									{isOpen && (
										<EmojiPicker onEmojiSelected={handleSelect} open={isOpen} onClose={() => setIsOpen(false)} />
									)}
								</TouchableOpacity>
